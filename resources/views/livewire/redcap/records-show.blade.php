<div>
    ...
    @endif
    <table
        class="border-collapse border border-indigo-600 items-start align-baseline mx-auto">
        <tr>
            <td class="border border-indigo-400 text-xl px-4">Participant ID
            </td>
            <td class="border border-indigo-400 text-xl px-4">Group</td>
            @if($field['randTime']??'')
                <td class="border border-indigo-400 text-xl px-4">Randomisation
                    time
                </td>
            @endif
        ...
        @forelse($records as $record)
            <tr>
                <td class="border border-indigo-400 px-4">
                    <a href=" {{route('redcap.record', ['record_id' => $record[$field['recordId']] ])}}">
                ...
                <td class="border border-indigo-400 px-4 text-center">
                    @if($field['randGroup'] && $record[$field['randGroup']]=='')
                        <a class="text-green-500 hover:text-red-500"
                           href="{{route('redcap.randomise', ['record_id' => $record[ $field[ 'recordId'] ] ] ) }}">
                            randomise
                        </a>
                    @else
                        <a class="text-green-500 hover:text-red-500"
                           href="{{route('redcap.record', ['record_id' => $record[$field['recordId']] ])}}">
                            view
                        </a>
                    @endif
                </td>
