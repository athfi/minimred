<div>
    <div class="mt-10 sm:mt-0">
                ...
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <img class="w-8 h-8 rounded-full" src="{{ $project->owner->profile_photo_url }}" alt="{{ $user->name }}">
                            <div class="ml-4">{{ $project->owner->name }}</div>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center">
                                ...
                                @if($project->isOwnerOrSupperAdmin(Auth::user()))
                                    <button class="ml-2 text-sm text-gray-400 underline" wire:click="manageRole('{{ $user->id }}')">
                                        {{collect($this->roles)->firstWhere('key', $user->projectRole($project))['name']}}
                                    </button>
                                @else
                                    <div class="ml-2 text-sm text-gray-400">
                                        {{collect($this->roles)->firstWhere('key', $user->projectRole($project))['name']}}
                            ...
                            <!-- Leave Team -->
                                @if ($this->user->id === $user->id)
                                    <button class="cursor-pointer ml-6 text-sm text-red-500" wire:click="confirmLeavingTeam('{{ $user->id }}')">
                                        {{ __('Leave') }}
                                    </button>

                                    <!-- Remove Team Member -->
                                @elseif ($project->isOwnerOrSupperAdmin(Auth::user()))
        ...
        <x-jet-confirmation-modal wire:model="confirmingLeavingTeam">
            ...
            <x-slot name="content">
                @if($teamMemberIdBeingRemoved == Auth::user()->id)
    ...
    <x-jet-form-section submit="add">
        ...
        <x-slot name="description">
            {{ __('Add a new team member to your project.') }}
        </x-slot>

        <x-slot name="form">
            <div class="col-span-6">
                <div class="max-w-xl text-sm text-gray-600">
                    {{ __('Please select the email address of the person you would like to add to this team.') }}
                </div>
            </div>
            ...
            <div class="col-span-6 sm:col-span-4">
                ...
                <x-jet-input id="email" type="text" class="mt-1 block w-full"
                             wire:model.debounce.200ms="addTeamMemberForm.email" autofocus/>
