    ...
    <x-jet-form-section submit="updateSetting">
        ...
        <x-slot name="form">
            ...
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="distance_method" value="Distance method"/>
                <select
                    wire:model="distance_method"
                    ...
                    type="text"
                >
                    @foreach( $distance_list as $key => $label )
                        <option
                            value={{ $key }}
                        >{{ $label }} </option>
                    @endforeach
                </select>
                <x-jet-input-error for="distance_method" class="mt-2"/>
            </div>
